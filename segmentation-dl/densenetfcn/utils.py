...
def display_loss_metric_curves(nb_epochs, history, save_fig_dir, save_fig_filename):
    ...
    ax[1].plot(time_steps, train_dice, 'm', label='IOU')
   # ax[1].plot(time_steps, val_dice, 'b', label='Valid')
    ...
    ax[2].plot(time_steps, train_accuracy, 'm', label='Accuracy')
    # ax[1].plot(time_steps, val_dice, 'b', label='Valid')
    ax[2].legend()
...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.train_metrics.append(logs.get('IOU'))
        self.train_accuracy.append(logs.get('acc'))
        self.val_metrics.append(logs.get('val_multiclass_dice'))

def display_multi_curves(nb_epochs, history, save_fig_dir, save_fig_filename):

    """Display multi curves in one graph, for example, metrics under different learning rate."""

    # Retrieve values from the log
    time_steps = range(nb_epochs)
    train_loss = history.train_losses#loss
    train_dice = history.train_metrics#IOU
    train_accuracy = history.train_accuracy#accuracy
