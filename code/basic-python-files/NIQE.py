...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2 = Image.fromarray(img).resize((round(h/2), round(w/2)))
    img2 =np.asarray(img2)
    #img2 = np.resize(img,((round(h/2), round(w/2))))#this method is bad!
...
if __name__ == "__main__":
    ref = np.array(Image.open('./test.png').convert('LA'))[:, :, 0]  # ref
