...
image = nd.imread('/home/jing/Desktop/imgseg/data/skin1.jpg', flatten=True)
# Initial level set
#init_ls = checkerboard_level_set(image.shape, 6)
init_ls = np.zeros(image.shape, dtype=np.int8)
init_ls[800:1100, 450:600] = 1
# List with intermediate results for plotting the evolution
evolution = []
callback = store_evolution_in(evolution)
ls = morphological_chan_vese(image, 200, init_level_set=init_ls, smoothing=3,
                             iter_callback=callback)
print(ls)
pilutil.Image.fromarray(ls, mode='L')
# b is converted from an ndarray to an image
b = scipy.misc.toimage(ls)
b.save('morph_c_v.png')
b.show()
